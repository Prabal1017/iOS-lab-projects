...
class Spaceship {
    ...
    func wasHit()->String
    {
        var result="";
        health-=5;
        result=health<=0 ? "Sorry, your ship was hit one too many times. Do you want to play again?" : "Current Health \(health)";
        return result;
    }
}
...
let falcon="Falcon";
var falconObject=Spaceship(name:falcon);
print(falconObject.name);
...
falconObject.moveLeft();
print("New Position : \(falconObject.position)");
falconObject.moveLeft();
print("New Position : \(falconObject.position)");
falconObject.moveRight();
print("New Position : \(falconObject.position)");
